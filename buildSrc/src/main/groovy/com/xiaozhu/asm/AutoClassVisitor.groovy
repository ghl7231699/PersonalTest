...
/**
 * 类描述：类的遍历，遍历其中方法，满足两个条件才能修改方法字节码：
 *       1、类要匹配
 *       2、方法匹配
 * 创建人：ghl
 * 创建时间：2019/4/2
...
class AutoClassVisitor extends ClassVisitor {
    ...
    //保存该类中的所有的方法
    private HashSet<String> visitedActivityMethods = new HashSet<>()
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        ...
        // 打印调试信息
        Logger.info("||---start scan method：${mClassName}")
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        if (GlobalConfig.isOpenLogTrack) {
            MethodVisitor logMethodVisitor = new LogMethodVisitor(methodVisitor, access, name,
                    desc, mSuperName, mClassName, mInterfaces, visitedFragMethods, visitedActivityMethods)
...
//        }
    ...
    void visitEnd() {
        //在类中追加方法
        //如果是启动页activity
        if (LogAnalyticsUtil.isLauncherActivity(mClassName)) {
            MethodVisitor mv
            Iterator<Map.Entry<String, LogMethodCell>> iterator = LogHookConfig.sActivityMethods.entrySet()
                    .iterator()
            while (iterator.hasNext()) {
                ...
                mv.visitCode()
                // 调用super方法
                LogAnalyticsUtil.visitMethodWithLoadedParams(mv, Opcodes.INVOKESPECIAL, mSuperName,
                        methodCell.name, methodCell.desc, methodCell.paramsStart, methodCell.paramsCount, methodCell.opcodes)
                // 插入方法
        ...
        Logger.info("||---end scan class :\t${mClassName}\n")
        super.visitEnd()
    }
//
//    /**
