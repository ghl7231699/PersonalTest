...
/**
 * 类名称：RoundImageViewByXfermode 类描述：自定义view用以按照圆形或者圆角四边形显示图形 创建人：gaoziqiang
 * 创建时间：2015年9月18日 下午3:13:45 修改人：gaoziqiang 修改时间：2015年9月18日 下午3:13:45 修改备注：
 *
 * @version v1.0
 */
public class RoundImageViewByXfermode extends ImageView {
    ...
    public void setType(int type) {
        if (TYPE_CIRCLE != type && TYPE_ROUND != type) {
            return;
        }
        this.type = type;
    }
    ...
    public RoundImageViewByXfermode(Context context, AttributeSet attrs) {
        ...
        mBorderRadius = a.getDimensionPixelSize(
                R.styleable.RoundImage_borderRadius,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                        BODER_RADIUS_DEFAULT, getResources()
                                .getDisplayMetrics()));// 默认为10dp
        type = a.getInt(R.styleable.RoundImage_type, TYPE_ROUND);// 默认为ROUND
